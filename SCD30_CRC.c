/*
 * CRC.c
 *
 * Created: 18.02.2021 16:54:52
 *  Author: Marvin
 */ 
//Calculate CRC-Value based on lookup table

#include "SCD30_CRC.h"


 //Generated lookup table
 const uint8_t crcLookupTable_u8[] = {
	 
 };
// Calculate CRC_8 for 16 bit data
uint8_t calc_CRC_8(uint16_t value)
{
	//Turn uint16_t in uint8_t array
	uint8_t data[2];
	data[0] = (value >> 8);	
	data[1] = value & 0xff;
	
	
	uint8_t crc = CRC_INIT; //Initial value according to datasheet
	for (uint8_t i = 0; i < 2; i++)
	{
		uint8_t x = (data[i] ^ crc);
		crc = crcLookupTable_u8[x];
	}
	return crc;
}
//Algorithm used to calculate table - not used
/*void calc_Table_CRC_8()
{
	for (uint16_t divident = 0; divident < 256; divident++)
	{
		uint8_t currByte = divident;
		for (uint8_t bit = 0; bit < 8; bit++)
		{
			if ((currByte & 0x80) != 0)
			{
				currByte <<= 1;
				currByte ^= CRC_POLY;
			}
			else
			{
				currByte <<= 1;
			}
		}
		crcLookupTable_u8[divident] = currByte;
	}
}*/