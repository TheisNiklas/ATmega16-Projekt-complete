...
I2C_Error_t I2C_Read(struct I2C_TxData_t TxData_st, uint8_t delay_in_ms_u8, uint8_t number_of_read_bytes)
{
	...
	else
	{
		...
		if (front == -1) //Schlange ist leer, Job direkt ausführen
		{
			...
			if (error == I2C_ERROR)
			{
				 // Mappen des Grundtreiber error auf einen Übertragungserror
				 newJob.callback_function(I2C_TXRXERROR);
				 return I2C_NOERROR; // Hier keinen Fehler zurückgeben, da der Fehler im Grundtreiber und nicht in der Warteschlange aufgetreten ist
			}
			front = 0;
			rear = (rear + 1) % MAXSIZE;
			liste[rear] = newJob;
